...
* **круговой слайдер** (`pcm_circular_slider.dart`) — крупная цифра текущей
  температуры, вокруг — регулировка установки. Подписи: `presetTemperature`,
  `standby`, `notConnect`;
...
Пока `isWorking`, приложение блокирует изменение установки и параметров и
...
1. если установка температуры менялась и устройство не в состоянии
...
* **°C / °F** — флаг `TemperatureUnitIsF` в SharedPreferences. Локальный только
  выбор единицы показа; сам протокол смешанный: текущая температура `0x00[3]`
  приходит в Цельсиях, а установка и её границы (`0x15`, `0x16`) — в
  Фаренгейтах. Приложение внутри держит всё в °F и переводит в °C только для
  экрана. Подробности — §6.3.1 протокола;
...
6. команды `0x02` (пуск/стоп) и `0x16` (установка температуры).
...
getTempSetting(0x15)               -> min/max/установка температуры
...
Stop. Установка температуры и параметры заваривания редактируются только в этом же
состоянии.
