...
| `0x16` | `setTempSetting` | → | 1 байт: `[temp]` | 1 байт: `[temp]` |
...
| `0x20` | `reset` | → | — | 5 байт (сброшенные параметры) |
...
| 3 | текущая температура, °C (в °F пересчитывается как `round(C * 1.8 + 32)`) |
...
`0x18` (`setDeviceWorkParam`) формируется в двух вариантах — в зависимости от
флага «устройство поддерживает регулировку давления» (`PcmDevice.field_6f`):

* флаг взведён → 4 байта;
* иначе → 3 байта (без давления).

Ответ на `0x18` (как и на `0x20`) содержит эхо текущих значений: байт 0 —
давление (клампится по min/max), байты 1–3 — три времени в том же порядке, что
в `0x17`. По этому эху и следует восстанавливать порядок байт запроса
`[давление, замачивание, выстаивание, экстракция]`.

### 6.3 `0x15` / `0x16` — уставка температуры

Ответ на `0x15`, 3 байта:

| Байт | Поле |
|---|---|
| 0 | минимально допустимая температура, °C |
| 1 | максимально допустимая температура, °C |
| 2 | текущая уставка (`presetTemperature`), °C |

`0x16` — 1 байт: новая уставка в °C. Ответ — 1 байт с принятым значением.
...
| `0x02` | `setTempSetting` | 1 байт: уставка |
...
* порядок байт в запросе `0x18` восстановлен по эху в ответе, а не напрямую;
